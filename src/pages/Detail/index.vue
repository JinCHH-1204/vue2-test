        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <!-- 基础信息选项 -->
              <dl
                v-for="(skuSaleAttr, index) in skuSaleAttr"
                :key="skuSaleAttr.id"
              >
                <dt class="title">{{ skuSaleAttr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  v-for="(saleAttr, index) in skuSaleAttr.spuSaleAttrValueList"
                  :key="saleAttr.id"
                  :class="{ active: saleAttr.isChecked == 1 }"
                  @click="
                    changeActive(saleAttr, skuSaleAttr.spuSaleAttrValueList)
                  "
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 购买个数 -->
                <!-- @change  表单事件 用于监听元素的改变 -->
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="skuNum"
                  @change="changeSkuValue"
                />
...
export default {
  ...
  computed: {
    ...mapGetters(["categoryView", "skuInfo", "skuSaleAttr"]),
    skuImageList() {
  ...
  methods: {
    ...
    async addOrUpdateShopcart() {
      ...
      try {
        ...
        //成功 路由跳转
        //skuInfo 为对象 数据太复杂 可以传入query 但不太合适
        //采用 会话存储sessionStorage 存储  只需展现一次，无需长时间保持
        sessionStorage.setItem("ADDORUPDATE", JSON.stringify(this.skuInfo));
        this.$router.push({
          name: "addcartsuccess",
